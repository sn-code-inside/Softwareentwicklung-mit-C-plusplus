...
void CLogTool::defineLogSrvIniData(void)
{
    ...
    strcpy_s(logSrvIniData_.sMapName[iLs], SIZE, "SHAREDMEM_IR");
    ...
    strcpy_s(logSrvIniData_.sMapName[iLs], SIZE, "SHAREDMEM_SR");
    strcpy_s(logSrvIniData_.sLogFileName[iLs], MAX_PATHNAME_LEN, "C:\\LOGSENDER\\LOG\\");
...
int CLogTool::logCreateSharedMemory(tLogOneSection* p2LogOneSec)
{
    ...
    if (p2LogOneSec->iInitialized == 0)                                                         //0: Abschnitt wurde noch nicht initialisiert
    {
        ...
        mutex_->lockMutex();                                                                    //Jetzt wird der Bereich gelockt     
        if (ret != 0)                                                                           //Der Lock sichert, dass nur ein Mal CreateFileMapping aktiviert wird
            return ret;

        p2LogOneSec->hMemMapFile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, (DWORD)iShmSize,
                                                    (LPCWSTR)p2LogOneSec->sMapName);
...
int CLogTool::logOpenSharedMemory(tLogOneSection* p2LogOneSec)
{
    ...
    if (p2LogOneSec->iInitialized == 0)
    {
        ...
        p2LogOneSec->hMemMapFile = OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, 
                                                  (LPCWSTR) p2LogOneSec->sMapName);
...
/* Out  : 0: OK; -1: LogSection-Vereletzung; -2: Keine Initialisierung       */
/*        int* iLineCount: Anz gelesener Zeilen; char** p2p2Lines: Inhalt    */
/*===========================================================================*/
int CLogTool::logRead(int iLs, int* iLineCount, char** p2p2Lines)
{
    int lines = 0;
    int i = 0;
    *iLineCount = 0;
    ...
    mutex_->lockMutex();                                                                        //Lock Bereich, damit nur eine Applikation zugreifen kann
    ...
    mutex_->unlockMutex();                                                                      //Jetzt kann der Lock-Bereich wieder frei gegeben werden  
    return 0;
}
...
int CLogTool::logWrite(enLogLevelType logLevel, int iLs, const char* sFunction, char* sFormat, ...)
{
    ...
    if (logLevel > enError)
        return 0;
    if (getGlobalLogLevel() == enNoLog)
        return 0;                                                                               //Bei GlobalLogLevel==enNoLog kein Schreiben
    else
    {
        if (!((getGlobalLogLevel() == enInfo) && (logLevel == enInfo)))                         //Bei GlobalLogLevel && logLevel = enInfo Schreiben
        {
            if (!((getGlobalLogLevel() == enError) && (logLevel >= enInfo)))                    //Bei GlobalLogLevel enError && logLevel >= enInfo Schreiben
                return 0;                                                                       //ansonsten: Kein Schreiben
        }
    }
    ...
    case enError:
        ...
        break;
    default:
        strcpy_s(sLogLevel, 20, "WrongLogLevel");
        break;
    }
    ...
    case 2:
        ...
        break;
    default:
        strcpy_s(sLogSect, 50, "WrongSection");
        break;
    }
    ...
    while (iBytesToTransfer > 0)
    {
        ...
        sprintf_s(sMessage, LOG_MESSAGE_SIZE, "%02d.%02d.%04d;%02d:%02d:%02d;%03d;%s;%s;%s;%s;%s",
            iDay, iMonth, iYear, sysTime.wHour, sysTime.wMinute, sysTime.wSecond, sysTime.wMilliseconds,
        ...
        if (logAllSection_[iLs].p2ShmBuf->iAktLine > LOG_MAX_LINES)
        {
            logAllSection_[iLs].p2ShmBuf->iAktLine--;                                           //Muss decr. werden; ansonsten Exception beim logServer
            mutex_->unlockMutex();                                                              //Achtung: Es ist sicherlich der Sache dienlich, den Start der 
            return -3;                                                                          //         logServers nach dem Start des logSenders zu "erzwingen".
        }
...
void CLogTool::setGlobalLogLevel(enLogLevelType enLLevel)
{
    iGlobalLogLevel_ = enLLevel;
}

enLogLevelType CLogTool::getGlobalLogLevel(void)
{
    return iGlobalLogLevel_;
}