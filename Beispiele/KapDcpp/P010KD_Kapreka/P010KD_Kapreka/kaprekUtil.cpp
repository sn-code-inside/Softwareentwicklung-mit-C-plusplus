...
void kaprekUtil::traceData(void)
{
    ...
    int iIdx = 0;
    while (iIdx < (getMaxKap ()-1))
    {
        if (allKaprek_[iIdx].iNumb == 998)
        {
            int i = 0;
        }
        if (allKaprek_[iIdx].iNumb)
        {
...
// Out  : -
//******************************************************************************
void kaprekUtil::traceKapTryData(void)
{
    FILE* fp = NULL;
    if (getKapWidth() == KAP_WIDTH_1000)
        fp = fopen("DataKapTry_1000.csv", "w");
    else if (getKapWidth() == KAP_WIDTH_100)
        fp = fopen("DataKapTry_100.csv", "w");
    else
        fp = fopen("DataKapTry.csv", "w");
