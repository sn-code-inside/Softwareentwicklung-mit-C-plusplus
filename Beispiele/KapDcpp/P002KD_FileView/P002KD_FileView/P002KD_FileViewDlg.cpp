...
void setFileName(CString str) {	FName_ = str;}
CString getFileName(void) {return FName_;}
...
void CP002KDFileViewDlg::writeFileDataToEditControl()
{
	if (FName_ != L"")
	{
		cFileUtil myFile(FName_);
		char* strFileData = myFile.getFileData();
		strFNameAndPath_ = FName_;

		CWnd* p2EditControl = GetDlgItem(IDC_EDIT_TEXT);
		CString strWinText(strFileData);
		p2EditControl->SetWindowTextW(strWinText);
	}
}
